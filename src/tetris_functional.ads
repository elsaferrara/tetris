...
is
   ...
   subtype X_Coord is Interfaces.Integer_16 range 1 .. X_Size;
   subtype Y_Coord is Interfaces.Integer_16 range 1 .. Y_Size;
   ...
   subtype PX_Coord is Integer_16 range -1 .. X_Size - 1;
   subtype PY_Coord is Integer_16 range -1 .. Y_Size - 1;
   ...
   --    shapes except I and O have 4 orientations, which all fit in the
   --     3 x 3 box
   ...
   subtype I_Delta is Integer_16 range 0 .. 3;
   type Oriented_I_Shape is array (I_Delta, I_Delta) of Boolean;
   subtype Three_Delta is Integer_16 range 0 .. 2;
   ...
   Possible_Three_Shapes : constant array (Three_Shape, Direction)
     of Oriented_Three_Shape :=
     [ --  orientations for J
   ...
   function Is_Empty (B : Board; Y : Integer_16; X : Integer_16) return Boolean
   is (X in X_Coord and then Y in Y_Coord and then B (Y)(X) = Empty);

   function Is_Complete_Line (L : Line) return Boolean
   is (for all X in X_Coord => L (X) /= Empty);

   function Is_Empty_Line (L : Line) return Boolean
   is (for all X in X_Coord => L (X) = Empty);

   function No_Complete_Lines (B : Board) return Boolean
   is (for all Y in Y_Coord => not Is_Complete_Line (B (Y)));
   ...
   is (case P.S is
          when O => Is_Empty (B, P.Y, P.X)
                    and then Is_Empty (B, P.Y, P.X + 1)
                    and then Is_Empty (B, P.Y + 1, P.X)
                    and then Is_Empty (B, P.Y + 1, P.X + 1),
         when I =>
           (for all Y in I_Delta =>
              (for all X in I_Delta =>
                   (if Possible_I_Shapes (P.D) (Y, X)
                    then Is_Empty (B, P.Y + Y, P.X + X)))),
         when Three_Shape =>
           (for all Y in Three_Delta =>
              (for all X in Three_Delta =>
                   (if Possible_Three_Shapes (P.S, P.D) (Y, X)
   ...
   function Valid_Configuration return Boolean is
     (Piece_Within_Board (Cur_Piece) and then
      (case Cur_State is
   ...
   type Action is (Move_Left, Move_Right, Move_Down, Turn_Counter_Clockwise,
                   Turn_Clockwise);

   subtype Move_Action is Action range Move_Left .. Move_Right;
   subtype Turn_Action
     is Action range Turn_Counter_Clockwise .. Turn_Clockwise;

   function Turn_Direction (D : Direction; T : Turn_Action) return Direction is
      (case T is
         when Turn_Counter_Clockwise => (if D = Direction'First
                                         then Direction'Last
                                         else Direction'Pred (D)),
          when Turn_Clockwise  => (if D = Direction'Last
                                   then Direction'First
                                   else Direction'Succ (D)));
   ...
   procedure Delete_Complete_Lines (Number_Of_Complete_Lines : out Unsigned_32)
     with
